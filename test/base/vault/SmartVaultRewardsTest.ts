...
import { DepositHelper, Multicall, NoopStrategy } from '../../../typechain';
...
describe('Smart vault rewards test', () => {
  ...
  before(async function() {
    ...
    await core.controller.setRewardDistribution([signer.address], true);
    await core.controller.changeWhiteListStatus([signer.address], true);
  ...
  it('check reward vesting for multiple accounts with MATIC rewards and LP underlying', async() => {
    const underlying = (await DeployerUtils.deployMockToken(signer, 'UND', 18)).address.toLowerCase();
    ...
    await core.controller.changeWhiteListStatus([
      user1.address,
      user2.address,
      user3.address,
      user4.address,
    ...
    console.log(
      'underlying amount',
      utils.formatUnits(await TokenUtils.balanceOf(underlying, signer.address), underlyingDec),
    );
    ...
    await TokenUtils.transfer(
      ...
      signer,
      user6.address,
      utils.parseUnits(user6Deposit, underlyingDec).toString(),
    );
    await TokenUtils.transfer(
      underlying,
      signer,
      user7.address,
      utils.parseUnits(user7Deposit, underlyingDec).toString(),
    );
    ...
    for (let i = 0; i < cycles; i++) {
      ...
      const toClaimUser5 = +utils.formatUnits(await vault.earned(rt, user5.address), rtDecimals);
      ...
      const toClaimUser6 = +utils.formatUnits(await vault.earned(rt, user6.address), rtDecimals);
      ...
      const _toClaimUser3 = +utils.formatUnits(await vault.earned(rt, user3.address), rtDecimals);
      ...
      const claimedUser5 = +utils.formatUnits(await TokenUtils.balanceOf(rt, user5.address), rtDecimals) -
        rtBalanceUser5;
      ...
      const claimedUser6 = +utils.formatUnits(await TokenUtils.balanceOf(rt, user6.address), rtDecimals) -
        rtBalanceUser6;
      ...
      if (user1Deposited) {
        ...
        await vault.connect(user1).exit();
        await VaultUtils.deposit(user1, vault, utils.parseUnits(user1Deposit, underlyingDec));
        const claimedUser1 = +utils.formatUnits(await TokenUtils.balanceOf(rt, user1.address), rtDecimals) -
          rtBalanceUser1;
      ...
      if (user2Deposited) {
        ...
        const claimedUser2 = +utils.formatUnits(await TokenUtils.balanceOf(rt, user2.address), rtDecimals) -
          rtBalanceUser2;
      ...
      if (i !== 0 && (i % +((cycles / 2).toFixed()) === 0)) {
        ...
        const _claimedUser3 = +utils.formatUnits(await TokenUtils.balanceOf(rt, user3.address), rtDecimals) -
          _rtBalanceUser3;
    ...
    const toClaimUser3 = +utils.formatUnits(await vault.earned(rt, user3.address), rtDecimals);
    ...
    const toClaimUser4 = +utils.formatUnits(await vault.earned(rt, user4.address), rtDecimals);
    ...
    const toClaimUser7 = +utils.formatUnits(await vault.earned(rt, user7.address), rtDecimals);
    ...
    expect(toClaimUser7)
      .is
      .approximately(claimedUser7, claimedUser7 * 0.01, 'user7 (used DepositHelper) claimed not enough ');
  ...
  it('check reward with transfers', async() => {
    ...
    for (let i = 0; i < cycles; i++) {
      ...
      // signer claim
      const _toClaimSigner = +utils.formatUnits(await vault.earned(rt, signer.address), rtDecimals);
      ...
      const _claimedSigner = +utils.formatUnits(await TokenUtils.balanceOf(rt, signer.address), rtDecimals) -
        _rtBalanceSigner;
    ...
    const claimedSigner = +utils.formatUnits(await TokenUtils.balanceOf(rt, signer.address), rtDecimals) -
      rtBalanceSigner;
